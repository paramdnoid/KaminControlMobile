...
- `npm run claude:validate-settings`: full JSON Schema validation for `.claude/settings.json` and `.claude/settings.local.json` when present, plus guard-hook simulations.
...
- `CLAUDE.md`, `.claude/**`, `.gitignore`: parse `.claude/settings.json`, run `npm run claude:validate-settings`, compile `.claude/hooks/*.py`, verify agent `skills:` links, verify skill frontmatter policy, `git diff --check -- CLAUDE.md .claude .gitignore`.
